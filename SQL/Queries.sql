...
CREATE TABLE PRS_PERSONS (
		...
		CONSTRAINT UN_USR_EMAIL UNIQUE(USR_EMAIL);			-- UNIQUE for usr_email
);
	/	
...
CREATE TABLE AUC_AUCTIONS (
		...
		AUC_END_TIME DATE NOT NULL,
		AUC_FK_USER_ID NUMBER(8) NOT NULL,
		AUC_FK_STATUS_ID NUMBER(1) NOT NULL,
		AUC_FK_PRODUCT_ID NUMBER(8) NOT NULL,
...
CREATE TABLE STA_STATUSES (
		STA_PK_STATUS_ID NUMBER(1),
		STA_DESCRIPTION VARCHAR(32),
		STA_FK_RIGHT_ID NUMBER(3),
		
		CONSTRAINT STA_PK_STATUS_ID 
			PRIMARY KEY (STA_PK_STATUS_ID))	;					-- PK of Status		
);

CREATE TABLE PRD_PRODUCTS (
		PRD_PK_PRODUCT_ID NUMBER(8),
		PRD_NAME VARCHAR(128) NOT NULL,
		PRD_DESCRIPTION VARCHAR(512),
		PRD_PHOTO_URL VARCHAR (64),
		PRD_FK_CATEGORY_ID NUMBER(2),
		
		CONSTRAINT PRD_PK_PRODUCT_ID 
			PRIMARY KEY (PRD_PK_PRODUCT_ID))	;					-- PK of Producs		
);
